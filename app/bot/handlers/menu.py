...
async def _send_qr(message, employee, lang):
...
async def menu_events(message: Message, employee: dict, lang: str):
    ...
    for ev in events:
        ...
        pool_amount = ev.get("reward_pool_amount", "")
        pool_currency = ev.get("reward_pool_currency", "")
        pool_line = f"\n💰 {t('event.pool', lang).format(amount=pool_amount, currency=pool_currency)}" if pool_amount else ""
        text = (f"🏆 <b>{ev.get('event_name')}</b>\n"
                f"🆔 <code>{ev.get('event_code')}</code>\n"
                f"{ev.get('description', '')}\n\n"
                f"📅 {ev.get('start_at')} — {ev.get('end_at')}\n"
                f"📋 {ev.get('rules_text', '')}{status_line}{pool_line}\n\n"
                f"🎁 Mukofotlar:\n{rewards_text or '-'}")
