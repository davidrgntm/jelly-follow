...
from app.services.admins_service import is_admin, set_admin_language
...
def _menu_markup(lang, user_id):
    return main_menu_keyboard(
        ...
        qr_webapp_url=_qr_webapp_url(user_id),
    )
...
BONUS_TITLE = {
    "uz": "✨ Qo'shimcha topshiriqlar",
    "ru": "✨ Дополнительные задания",
    "en": "✨ Bonus tasks",
    "kg": "✨ Кошумча тапшырмалар",
    "az": "✨ Əlavə tapşırıqlar",
}
...
def _format_bonus_rules(bonus_rules, lang: str) -> str:
    if not bonus_rules:
        return "-"
    lines = []
    for rule in bonus_rules[:8]:
        title = (rule.get("title") or "").strip() or "Bonus"
        points = str(rule.get("points") or "0").strip()
        min_followers = str(rule.get("min_followers") or "").strip()
        extra = f" • от {min_followers} подписчиков" if min_followers and min_followers not in {"", "0"} else ""
        lines.append(f"• {title} — <b>+{points}</b>{extra}")
    return "\n".join(lines) or "-"
...
def _render_event_text(ev: dict, lang: str) -> str:
    ...
    bonus_text = _format_bonus_rules(ev.get("bonus_rules", []), lang)
    ...
    if bonus_text != "-":
        lines.extend(["", f"{_lang_label(BONUS_TITLE, lang)}:", bonus_text])
...
async def cb_change_lang(cb: CallbackQuery, employee: dict, lang: str):
    ...
    if is_admin(cb.from_user.id):
        set_admin_language(cb.from_user.id, new_lang)
