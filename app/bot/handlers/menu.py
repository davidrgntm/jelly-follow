...
def _menu_matcher(tx: str, lang_keys: list[str]) -> bool:
    ...
    for key in lang_keys:
        variants = [v.lower() for v in t(key, lang="uz", fallback_all=True)]
        if low in variants:
            return True
    return False
...
async def menu_link(message: Message, employee: dict, lang: str):
    ...
    await message.answer(f"🔗 <b>{link}</b>", parse_mode="HTML", reply_markup=main_menu_keyboard(lang))
...
async def menu_events(message: Message, employee: dict, lang: str):
    ...
    events = get_events_for_employee(employee_id=employee["employee_id"], country_code=employee["country_code"])

    if not events:
        await message.answer(t("events.list_empty", lang), reply_markup=main_menu_keyboard(lang))
        return
...
async def cb_event_join(cb: CallbackQuery, employee: dict, lang: str):
    ...
    await cb.message.edit_reply_markup(reply_markup=event_participate_keyboard(lang, event_id, "accepted"))
...
async def cb_event_decline(cb: CallbackQuery, employee: dict, lang: str):
    ...
    await cb.message.edit_reply_markup(reply_markup=event_participate_keyboard(lang, event_id, "declined"))
...
async def cb_change_lang(cb: CallbackQuery, employee: dict, lang: str):
    ...
    await cb.message.answer(t("generic.lang_changed", new_lang), reply_markup=main_menu_keyboard(new_lang))
    await cb.answer()
...
async def menu_help(message: Message, employee: dict, lang: str):
    await message.answer(t("help.text", lang), parse_mode="HTML", reply_markup=main_menu_keyboard(lang))
