...
from app.services.notifications_service import notify_event_started, send_message, send_localized_update_broadcast
...
def _admin_keyboard(lang="uz", role="ga"):
    rows = [
        ...
        [KeyboardButton(text=t("admin.menu.broadcast", lang)), KeyboardButton(text=t("admin.menu.send_update", lang))],
...
@router.message(_admin_text("admin.menu.send_update"))
async def admin_send_update(message: Message, lang: str, **kwargs):
    admin = _get_admin_info(message.from_user.id)
    if not admin:
        return
    await message.answer(t("admin.update.sending", lang))
    employees = get_all_employees()
    result = await send_localized_update_broadcast(employees)
    await message.answer(
        t("admin.update.done", lang).format(sent=result["sent"], failed=result["failed"], total=result["total"]),
        parse_mode="HTML",
        reply_markup=_admin_keyboard(lang, admin.get("role_code", "ga")),
    )
