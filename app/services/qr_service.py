...
def generate_employee_qr(employee_id, employee_code, country_code):
    ...
    if existing and existing.get("event_id") in ("", None, 0):
        return existing
    ...
    row = [qr_id, short_link, employee_id, employee_code, "", country_code,
           short_link, f"/static/qr/{filename}", "yes", now_str()]
    sheets.append_row(SHEET_QR_CODES, row)
    update_employee_qr(employee_id, qr_id)
    _log(sheets, "qr_created", "employee", employee_id, f"QR created: {qr_id}")
    return {"qr_id": qr_id, "short_link": short_link, "qr_image_path": filepath,
...
def generate_event_qr(employee_id, employee_code, event_id, country_code):
    ...
    existing = next(
        (qr for qr in sheets.find_records(SHEET_QR_CODES, "employee_id", employee_id)
         if qr.get("event_id") == event_id), None)
    if existing:
        return existing
    ...
    filename = f"{qr_id}.png"
    filepath = generate_qr_image(short_link, filename)
    row = [qr_id, short_link, employee_id, employee_code, event_id, country_code,
           short_link, f"/static/qr/{filename}", "yes", now_str()]
    sheets.append_row(SHEET_QR_CODES, row)
    _log(sheets, "qr_event_created", "employee", employee_id, f"Event QR: {qr_id}")
    return {"qr_id": qr_id, "short_link": short_link, "qr_image_path": filepath,
...
def get_event_qr_bytes(employee_id, event_id):
    ...
    qr = next((q for q in qrs if q.get("event_id") == event_id), None)
    if not qr:
        return None
    return generate_qr_bytes(qr["short_link"])
