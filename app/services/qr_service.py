...
from app.integrations.google_sheets import (
    get_sheets, SHEET_QR_CODES, SHEET_SYSTEM_LOGS,
)
...
def generate_employee_qr(employee_id: str, employee_code: str, country_code: str) -> dict:
    """Generate permanent QR for employee."""
    sheets = get_sheets()
    ...
    row = [
        qr_id, short_link, employee_id, employee_code,
        "", country_code, short_link,
        f"/static/qr/{filename}", "yes", now_str()
    ]
    sheets.append_row(SHEET_QR_CODES, row)
    update_employee_qr(employee_id, qr_id)

    _log(sheets, "qr_created", "employee", employee_id,
         f"QR created: {qr_id} for {employee_code}")
...
def generate_event_qr(employee_id: str, employee_code: str, event_id: str, country_code: str) -> dict:
    """Generate event-specific QR for employee."""
    sheets = get_sheets()

    short_link = f"{settings.TRACKING_DOMAIN}/r/{employee_code}?event={event_id}"
    seq = sheets.get_next_seq(SHEET_QR_CODES)
    qr_id = generate_qr_id(seq)
    filename = f"{qr_id}.png"
    ...
    row = [
        qr_id, short_link, employee_id, employee_code,
        event_id, country_code, short_link,
        f"/static/qr/{filename}", "yes", now_str()
    ]
    sheets.append_row(SHEET_QR_CODES, row)

    _log(sheets, "qr_event_created", "employee", employee_id,
         f"Event QR created: {qr_id} event={event_id}")
...
def get_qr_bytes(employee_id: str) -> Optional[bytes]:
    """Get QR bytes for sending via Telegram."""
    sheets = get_sheets()
