...
from app.integrations.google_sheets import (
    get_sheets, SHEET_POINT_TRANSACTIONS, SHEET_SYSTEM_LOGS,
)
...
def award_point(
    ...
    country_code: str,
    event_id: str = "",
    reason_code: str = "first_unique_device",
    points: int = 1,
    created_by: str = "system",
) -> str:
    """Award points. Returns point_tx_id."""
    sheets = get_sheets()
    seq = sheets.get_next_seq(SHEET_POINT_TRANSACTIONS)
    tx_id = generate_point_tx_id(seq)

    row = [
        tx_id, employee_id, employee_code,
        event_id or "", country_code,
        scan_id, device_key, points,
        reason_code, now_str(), created_by
    ]
    ...
    logger.info(f"Point awarded: {employee_id} +{points} [{reason_code}]")
    return tx_id
...
def manual_adjust(
    ...
    """Manual bonus or penalty by admin."""
    sheets = get_sheets()
    seq = sheets.get_next_seq(SHEET_POINT_TRANSACTIONS)
    tx_id = generate_point_tx_id(seq)
    ...
    row = [
        tx_id, employee_id, employee_code,
        event_id or "", country_code or "",
        "", "", points_delta,
        reason_code, now_str(), admin_id
    ]
    sheets.append_row(SHEET_POINT_TRANSACTIONS, row)
    ...
    logger.info(f"Manual point: {employee_id} {points_delta:+d} by {admin_id}")
    return tx_id


def get_employee_total_points(employee_id: str) -> int:
    sheets = get_sheets()
    txs = sheets.find_records(SHEET_POINT_TRANSACTIONS, "employee_id", employee_id)
    return sum(int(t.get("points_delta", 0)) for t in txs)


def get_employee_event_points(employee_id: str, event_id: str) -> int:
    sheets = get_sheets()
