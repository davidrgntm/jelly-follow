...
from app.integrations.google_sheets import (
    get_sheets, SHEET_ADMINS, SHEET_SYSTEM_LOGS, SHEET_EMPLOYEES, SHEET_META,
...
def get_admin_language(telegram_user_id, default="uz"):
    sheets = get_sheets()
    row = sheets.find_record(SHEET_META, "key", f"admin_lang:{telegram_user_id}")
    value = (row or {}).get("value", "").strip().lower()
    return value or default


def set_admin_language(telegram_user_id, language_code):
    sheets = get_sheets()
    key = f"admin_lang:{telegram_user_id}"
    row_idx = sheets.find_row_index(SHEET_META, "key", key)
    payload = {"key": key, "value": str(language_code).lower().strip(), "updated_at": now_str()}
    if row_idx:
        sheets.update_row(SHEET_META, row_idx, payload)
    else:
        sheets.append_row(SHEET_META, [payload["key"], payload["value"], payload["updated_at"]])
